...
void msc_dump_thread(void *param)
{
	...
	fic_dump_flag = 0;
	{
		...
		if (num == 0x18)
			sprintf(f_name, DUMP_PATH"tpeg_video_%04d.ts", index++);
		else if (num == 0x00)
			sprintf(f_name, DUMP_PATH"tpeg_audio_%04d.ts", index++);
		else
			sprintf(f_name, DUMP_PATH"tpeg_data_%04d.ts", index++);

		f = fopen(f_name, "wb");

		print_log(0, "TPEG start msc dump %s	%s SUBCH(0x%x)\n", f_name
...
//nexell 20160512
				...
				read_size = 0;
				print_log(0, "TPEG Total Read size ... %d Bytes %d\n", tot_read_size, size);
#endif
			}
