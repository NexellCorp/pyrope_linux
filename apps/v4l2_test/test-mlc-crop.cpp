...
int main(int argc, char *argv[])
{
	...
	if (argc == 8) {
		...
		start_x = atoi(argv[4]);
		start_y = atoi(argv[5]);
		rescale_width = atoi(argv[6]);
		rescale_height = atoi(argv[7]);
	} else {
		printf("usage: ./%s module width height start_x start_y rescale_width rescale_height\n", argv[0]);
		return 0;
	}
	...
	// setting source crop
	CHECK_COMMAND(v4l2_set_crop_with_pad(video_id, 2, start_x, start_y, rescale_width, rescale_height));
