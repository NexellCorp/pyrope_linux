...
public class CinemaService extends Service {
                ...
                while( mRun )
                {
                    ...
                    if( null != lSocket ) {
                        ...
                        if( strValue.equals("run") ) {
                            //ShowLoading( true );
                            new AsyncTaskLoading( true ).execute();
                        }
                        else if( strValue.equals("stop") ) {
                            //ShowLoading( false );
                            new AsyncTaskLoading( false ).execute();
                        }
                        else {
                            Log.i(VD_DTAG, "Invalid TMS Event.");
                        }
                        lSocket.close();
                    }
                }
    ...
    private class AsyncTaskLoading extends AsyncTask<Void, Void, Void> {
        ...
        public AsyncTaskLoading( boolean show ) {
            mShow = show;
        }
        ...
        protected void onPostExecute(Void aVoid) {
            if( !mShow ) {
                if( mTmsEventCallback != null )
                    mTmsEventCallback.onTmsEventCallback();
...
//    private void ShowLoading( final boolean enable ) {
