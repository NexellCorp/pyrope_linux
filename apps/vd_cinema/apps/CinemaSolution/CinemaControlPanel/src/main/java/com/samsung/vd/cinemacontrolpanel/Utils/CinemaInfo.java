...
public class CinemaInfo extends Application {
    ...
    public void onCreate() {
        ...
        Log.i(VD_DTAG , "onCreate+++++");
        NXAsync.getInstance().setAppContext(getApplicationContext());
        Log.i(VD_DTAG , "onCreate-----");
    ...
    public void onTerminate() {
        Log.i(VD_DTAG , "onTerminate+++++");
        super.onTerminate();

        if(NXAsync.getInstance().getIsThreadRunning()){
            try {
                NXAsync.getInstance().stopThread();
                Log.i(VD_DTAG , "onTerminate,, Thread of NXAsync is stopped");
    ...
    public String GetValue( String key ) {
        if(key != CinemaInfo.KEY_UPDATE_TGAM0 && key != CinemaInfo.KEY_UPDATE_TGAM1 && key != CinemaInfo.KEY_UPDATE_DGAM0 && key != CinemaInfo.KEY_UPDATE_DGAM1 ){
            return mPrefConfig.GetValue(key);
        }

        if(null == mPrefConfig.GetValue(key)) return "0";
        return mPrefConfig.GetValue(key);
    }
