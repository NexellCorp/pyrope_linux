...
static long dmb_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	default:
		print_log(hInit, "tdmb ioctl error! cmd(0x%x, 0x%x)\n", cmd, IOCTL_DMB_WORD_READ);
		res = BBM_NOK;
		break;
	}
 ...
 */
    ...
    nxp_soc_gpio_set_int_mode(CFG_GPIO_TDMB_INT, 2);
    ...
    nxp_soc_gpio_set_int_enable(CFG_GPIO_TDMB_INT, 0);

    //irq_set_irq_type(gpio_to_irq(CFG_GPIO_TDMB_INT), IRQF_TRIGGER_FALLING);
	//res = request_irq(gpio_to_irq(CFG_GPIO_TDMB_INT),  dmb_irq, 0, FC8080_NAME, NULL);

	//res = request_irq(gpio_to_irq(CFG_GPIO_TDMB_INT),  dmb_irq, IRQF_TRIGGER_FALLING , FC8080_NAME, NULL);

	res = request_irq(PB_PIO_IRQ(CFG_GPIO_TDMB_INT),  dmb_irq, IRQF_TRIGGER_FALLING , FC8080_NAME, NULL);
    ...
    {
//  		nxp_soc_gpio_set_io_pullup(CFG_GPIO_TDMB_INT, true);
//		nxp_soc_gpio_set_int_mode(CFG_GPIO_TDMB_INT, 4);
		nxp_soc_gpio_set_int_enable(CFG_GPIO_TDMB_INT, 1);
		nxp_soc_gpio_clr_int_pend(CFG_GPIO_TDMB_INT);
