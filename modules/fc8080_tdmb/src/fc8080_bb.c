...
#if defined(FC8080_MODE_DET)
	bbm_write(handle, BBM_OFDM_DET_MODE_EN, 0x0f);
#endif
...
s32 fc8080_ber_overrun_check(HANDLE handle, u32 *ber)
{
	u8 reset 	= 0x01;
	...
	u32 dmp_result;
        ...
        struct ioctl_info *localinfo;

        localinfo = container_of(ber, struct ioctl_info, buff[0]);

        BUG_ON(sizeof(localinfo->buff) != 16);
        localinfo->buff[1] = 0x0;

	/* fc8080 viterbi ber read function */
	bbm_write(handle, 0xe01, 0x0f);

	bbm_long_read(handle, 0xe40, &dmp_ber_rxd_bits);
	bbm_long_read(handle, 0xe44, &dmp_ber_err_bits);

	if (dmp_ber_rxd_bits)
          dmp_result = (dmp_ber_err_bits * 20000) / dmp_ber_rxd_bits;
	else
          dmp_result = 20000;
	...
	if (dmp_result <= 20)
        ...
        if (ber)
          localinfo->buff[0] = dmp_result;


	/* fc8080 internal buffer overrun check function */
	bbm_word_read(handle, BBM_BUF_OVERRUN, &overrun);
	...
	if (overrun & dmb_mode) {
		...
		if (reset) {
			/* buffer restore */
			bbm_word_read(handle, BBM_BUF_ENABLE, &temp);
			temp &= ~dmb_mode;
			bbm_word_write(handle, BBM_BUF_ENABLE, temp);
			temp |= dmb_mode;
			bbm_word_write(handle, BBM_BUF_ENABLE, temp);
		}
		print_log(handle, "fc8080 Overrun occured\n");
	}
