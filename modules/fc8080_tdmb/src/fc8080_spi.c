...
static s32 tdmb_spi_bulkwrite(HANDLE handle, u16 addr, u8 command, u8 *data,
			u16 length)
{
	s32 res = BBM_OK;
	s32 i = 0;
 ...
 //   print_log(0, "tdmb_spi_bulkwrite: %d\n", length);
...
static s32 tdmb_spi_dataread(HANDLE handle, u8 addr, u8 command, u8 *data,
			u16 length)
{
	s32 res = BBM_OK;
    ...
    if (res) {
		print_log(0, "fc8080_tdmb_spi_dataread fail : %d\n", res);
		return BBM_NOK;
	}
...
s32 fc8080_spi_wordread(HANDLE handle, u16 addr, u16 *data)
{
	...
	mutex_lock(&lock);
    //print_log(1, "s\n");
	res = tdmb_spi_bulkread(handle, addr, command, (u8 *) data, 2);
	mutex_unlock(&lock);
...
s32 fc8080_spi_longread(HANDLE handle, u16 addr, u32 *data)
{
	...
	mutex_lock(&lock);
	res = tdmb_spi_bulkread(handle, addr, command, (u8 *) data, 4);
	mutex_unlock(&lock);
...
s32 fc8080_spi_bulkread(HANDLE handle, u16 addr, u8 *data, u16 length)
{
	...
	for (i = 0; i < x; i++, addr += 255)
		res |= tdmb_spi_bulkread(handle, addr, command, &data[i * 255], 255);
	if (y)
		res |= tdmb_spi_bulkread(handle, addr, command, &data[x * 255], y);
	mutex_unlock(&lock);
...
s32 fc8080_spi_bytewrite(HANDLE handle, u16 addr, u8 data)
{
	...
	mutex_lock(&lock);
	res = tdmb_spi_bulkwrite(handle, addr, command, (u8 *) &data, 1);
	mutex_unlock(&lock);
...
s32 fc8080_spi_wordwrite(HANDLE handle, u16 addr, u16 data)
{
	...
	mutex_lock(&lock);
	res = tdmb_spi_bulkwrite(handle, addr, command, (u8 *) &data, 2);
	mutex_unlock(&lock);
...
s32 fc8080_spi_longwrite(HANDLE handle, u16 addr, u32 data)
{
	...
	mutex_lock(&lock);
	res = tdmb_spi_bulkwrite(handle, addr, command, (u8 *) &data, 4);
	mutex_unlock(&lock);
...
s32 fc8080_spi_bulkwrite(HANDLE handle, u16 addr, u8 *data, u16 length)
{
	...
	mutex_lock(&lock);
	for (i = 0; i < x; i++, addr += 255)
		res |= tdmb_spi_bulkwrite(handle, addr, command, &data[i * 255],
					255);
	if (y)
		res |= tdmb_spi_bulkwrite(handle, addr, command, &data[x * 255], y);
	mutex_unlock(&lock);
...
s32 fc8080_spi_dataread(HANDLE handle, u16 addr, u8 *data, u16 length)
{
	...
	mutex_lock(&lock);
	res = tdmb_spi_dataread(handle, addr, command, data, length);
	mutex_unlock(&lock);
