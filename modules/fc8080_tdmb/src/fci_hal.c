...
#include "../include/fc8080_i2c.h"
#include "../include/fc8080_ppi.h"
...
struct interface_port {
	...
	s32 (*dataread)(HANDLE handle, u16 addr, u8 *data, u16 size);
...
{
	if (fc8080_spi_byteread(handle, addr, data))
...
{
	if (fc8080_spi_wordread(handle, addr, data))
		return BBM_E_BB_READ;
...
{
	if (fc8080_spi_longread(handle, addr, data))
		return BBM_E_BB_READ;
...
{
	if (fc8080_spi_bulkread(handle, addr, data, length))
		return BBM_E_BB_READ;
...
{
	if (fc8080_spi_bytewrite(handle, addr, data))
...
{
	if (fc8080_spi_wordwrite(handle, addr, data))
		return BBM_E_BB_WRITE;
...
{
	if (fc8080_spi_longwrite(handle, addr, data))
		return BBM_E_BB_WRITE;
...
{
	if (fc8080_spi_bulkwrite(handle, addr, data, length))
		return BBM_E_BB_WRITE;
...
{
	if (fc8080_spi_dataread(handle, addr, data, length))
		return BBM_E_BB_WRITE;
