...
static int dmb_thread(void *hDevice)
{
	...
	print_log(hInit, "tpeg_kthread enter\n");
	...
	print_log(hInit, "tpeg_kthread exit\n");
...
//	print_log(1, "dmb_ioctl\n");
	...
	case IOCTL_DMB_DEINIT:
	{
			...
			if(fci_ringbuffer_free(&hOpen->RingBuffer) < len+8 )
			  {
				print_log(NULL,"tpeg buffer deinitialization !");
			  }
	...
	default:
		print_log(hInit, "tpeg ioctl error! cmd(0x%x, 0x%x)\n", cmd, IOCTL_DMB_WORD_READ);
		res = BBM_NOK;
		break;
	}
...
int dmb_init(void)
{
	...
	print_log(hInit, "tpeg_driver_version : v1.8.1\n");
	print_log(hInit, "tpeg_init - power_pin : 0x%x	reset_pin : 0x%x \
		irq_num : 0x%x	XTAL : %d\n"
		, power_pin, reset_pin, irq_num, FC8080_FREQ_XTAL);



	print_log(0, "tpeg_driver_version : v1.8.1\n");
	print_log(0, "tpeg_init - power_pin : 0x%x	reset_pin : 0x%x \
		irq_num : 0x%x	XTAL : %d\n"
		, power_pin, reset_pin, irq_num, FC8080_FREQ_XTAL);
	...
	if (res < 0) {
          print_log(hInit, "tpeg init fail : %d\n", res);
          kzfree(hInit);
          return res;
	}
	...
	platform_hw_setting();
/* POWER_ON and POWER_OFF operation are controlled by dmb driver */
/*	platform_hw_init(power_pin, reset_pin); */

	res = bbm_com_hostif_select(hInit, BBM_SPI);
	if (res) {
          print_log(hInit, "tpeg host interface select fail!\n");
          kzfree(hInit);
          goto error;
        }
...
/* Add TPEG interrupt pin setting */

    nxp_soc_gpio_set_int_mode(CFG_GPIO_TPEG_INT, 2);
    ...
    nxp_soc_gpio_set_int_enable(CFG_GPIO_TPEG_INT, 0); 

    //irq_set_irq_type(gpio_to_irq(CFG_GPIO_TPEG_INT), IRQF_TRIGGER_FALLING);
	//res = request_irq(gpio_to_irq(CFG_GPIO_TPEG_INT),  dmb_irq, 0, FC8080_NAME, NULL);

	//res = request_irq(gpio_to_irq(CFG_GPIO_TPEG_INT),  dmb_irq, IRQF_TRIGGER_FALLING , FC8080_NAME, NULL);

	res = request_irq(PB_PIO_IRQ(CFG_GPIO_TPEG_INT),  dmb_irq, IRQF_TRIGGER_FALLING , FC8080_NAME, NULL);

	if (res) {
          print_log(hInit, "tpeg request irq fail : %d\n", res);
    ...
    {
//  		nxp_soc_gpio_set_io_pullup(CFG_GPIO_TPEG_INT, true);
//		nxp_soc_gpio_set_int_mode(CFG_GPIO_TPEG_INT, 4);
		nxp_soc_gpio_set_int_enable(CFG_GPIO_TPEG_INT, 1);
		nxp_soc_gpio_clr_int_pend(CFG_GPIO_TPEG_INT);
	...
	if (!dmb_kthread)
          dmb_kthread = kthread_run(dmb_thread, (void *)hInit, "tpeg_thread");
