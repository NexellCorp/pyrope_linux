...
#define	VER_STR	"Nexell USB Downloader Version 1.2.1"
...
int NXP4330_ImageDownload( void );
...
static void usage(void)
{
	...
	printf("   -b [file name]      : second boot file name(NXP4330)\n");
	printf("   -t [processor type] : select target processor type\n");
	printf("      ( type : nxp2102 or nxp3200 or nxp4330 )\n" );
	printf("   -a [address]        : download address(optinal)\n");
	printf("   -j [address]        : jump address(optinal)\n");
	printf("\n");
	printf(" case 1. nxp4330 second boot download\n" ); 
...
//
static int NXP4330_SecondBootMode(void)
{
	FILE *fd_nsih = NULL;
	...
	unsigned int *NSIH;
	int read_size;
	unsigned int down_addr=0x40100000, start_addr=0x40100000;
	...
	send_buf = malloc( second_boot_send_size+1024 );
	...
	send_data( NEXELL_VID, NXP4330_PID, send_buf, second_boot_send_size );
...
static int NXP4330_ImageOnly(void)
{
	FILE *fd_image = NULL;
	unsigned char *send_buf=NULL;
	...
	send_data( NEXELL_VID, NXP4330_PID, send_buf, buf_size );
	free( send_buf );
...
static int NXP4330_ImageWithHeader(void)
{
	FILE *fd_nsih = NULL;
	FILE *fd_image = NULL;
	unsigned char *send_buf=NULL;
...
int NXP4330_ImageDownload( void )
{
	if( secondboot_file==NULL )
	{
		...
		{
			//	Header(512) + Image
			return NXP4330_ImageWithHeader();
		}
		else
		{
			//	Image Only Mode
			return NXP4330_ImageOnly();
		}
	}
	else
	{
		//	Second Boot Mode
		return NXP4330_SecondBootMode();
	}
	return 0;
}
//
